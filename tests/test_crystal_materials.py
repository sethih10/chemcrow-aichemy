...
@pytest.fixture
def cif_file_cof(tmp_path ):
    """Create a simple silicon structure for testing."""
    return str(tmp_path / "13030N2_ddec.cif")
...
def test_structure_symmetry_analysis(cif_file_tio2):
    """Test StructureSymmetryAnalysis tool."""
    tool = StructureSymmetryAnalysis()
    result = tool._run(cif_file_tio2)

    assert isinstance(result, str)
    assert "space_group" in result.lower()
    print(f"\n✓ StructureSymmetryAnalysis test passed")
    print(f"  Result: {result[:200]}...")


def test_structure_symmetry_analysis_cof(cif_file_cof):
    """Test StructureSymmetryAnalysis tool."""
    tool = StructureSymmetryAnalysis()
    result = tool._run(cif_file_cof)
...
def test_lattice_parameter_optimization(cif_file_si):
    ...
    assert isinstance(result, str)
    assert "lattice" in result.lower() 
...
def run_all_tests():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        tests = [
            ...
            (
                "LatticeParameterOptimization",
                lambda: test_lattice_parameter_optimization(str(cif_si)),
            ),
